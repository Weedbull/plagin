...
def  main():
  ...
  installer = "\installer\bullplug Installer.exe"
   
  if demo:
    installer = "\installer\bullplug Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\bullplug manual.pdf" 
  ]
  ...
  ZIPNAME = "bullplug-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "bullplug-v" + FULLVERSIONSTR + "-win-demo.zip"
